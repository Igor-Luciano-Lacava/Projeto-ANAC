-- ---------------------------------------------------------------------------
-- Passo 2 — o contrato de dados, escrito como codigo. --Contrato = regra de comportamento esperado. Construí um contrato --> cria regra de qualidade pra verificar se o contrato tá sendo respeitado
--
-- TODAS as expectations aqui sao "warn" (nenhuma tem ON VIOLATION), ou seja, não vamos dropar nenhuma linha se ela falhar na qualidade. Isso e
...
CREATE LIVE VIEW vra_auditado (
  -- === completude ===
  --constraint no contexto de pipelines é uma regra que construo de sql pra verificar se aquele valor esperado aconteceu ou nao. Se a condição não passar, ele marca um ponto de atenção de qualidade
